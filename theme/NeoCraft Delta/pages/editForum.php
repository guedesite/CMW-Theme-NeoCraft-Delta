...
if(isset($_Joueur_) AND isset($_GET['id'], $_GET['objet']))
{
	...
	if($AdminForum->verifEdit($objet, $id, $_Joueur_, $_PGrades_))
	{
		...
		<div class="neo-content-headline">
			<div class="neo-line-headline" style="width: 100%"><h2><em><strong><span style="font-size:40px">Edition d'un<?=($objet == 1) ? ' topic !': 'e réponse !';?></span></strong></em></h2></div>
		</div>
		<form action="?action=editForum" method="POST">
			<div class="container">
				...
				<button type="submit" class="neo-button neo-green" style="margin-bottom:15px;">Envoyer</button>
		      </div>
		  </form>
	      <?php 
	  }
