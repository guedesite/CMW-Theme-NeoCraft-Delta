			 ...
			 for($i = 0; $i < count($jsonCon); $i++) { ?>
				<div id="categorie-<?php echo $i; ?>" class=" neo-xbackground-50 neo-animate-opacity neo-margin-top-1 neo-margin-bottom-1 neo-tabs neo-radius neo-padding-16" style="display:none;padding:30px;">
							...
							if($data_count['count'] > 0)
							{
								...
								{
									?>
										<button type="submit" class="neo-button neo-green hvr-forward col-ms-3 col-lg-4 col-6" name="site" value="<?php echo $liensVotes['id']; ?>" onclick="window.open('<?php echo $liensVotes['lien']; ?>','Fiche','toolbar=no,status=no,width=1350 ,height=900,scrollbars=yes,location=no,resize=yes,menubar=yes')" >
								...
								echo '</br><p class="neo-text">Aucun lien de vote n\'est disponible pour ce serveur...</p>';
							?>
			...
			<div class="corp-bloc">
				...
				<table class="neo-table neo-striped neo-bordered neo-hoverable">
					...
					<thead>
						...
						<tr class="neo-white"><td><?php echo $i +1; ?></td><td><img src="<?=$Img->getImgToSize(30, $width, $height);?>" alt="" /> <strong><?php echo $topVoteurs[$i]['pseudo']; ?></strong></td><td><?php echo $topVoteurs[$i]['nbre_votes']; ?></td></tr>
						<?php }?>
				</table>
			</div>
	</div>
